...
ExcludedRanges.append( "169.254.0.0/16" )
ExcludedRanges.append( "192.168.0.0/16" )
ExcludedRanges.append( "172.16.0.0/12" )
ExcludedRanges.append( "10.0.0.0/8" )
...
ExcludedRanges.extend( ClassXRanges )

ExcludedRanges.extend( ClassARanges )

ExcludedRanges.extend( ClassBRanges )

ExcludedRanges.extend( ClassCRanges )

def ReturnCIDR( ExcludedRange ) :
    ExcludedRangeCIDR = int( ( ExcludedRange.split( "/" ) )[ 1 ] )
    return ( ExcludedRangeCIDR )

ExcludedRanges.sort( key=ReturnCIDR )

VPNExcludedRangesTXTFileObj = open( "OpenVPN-Client-Excluded-Ranges-" + DBIPCSVFileName[ -9 : -4 ] + ".txt" ,"w+" )
...
for ExcludedRange in ExcludedRanges :
    VPNExcludedRangesTXTFileObj.write( ExcludedRange + "\n" )
    VPNExcludedRangesRSCFileObj.write( "/ip firewall address-list add address=" + ExcludedRange + " list=VPN-Excluded-Ranges" + "\n" )

VPNExcludedRangesTXTFileObj.close()
